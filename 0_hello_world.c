...
int main(int argc, const char *argv[]) {
    if (argc < 2) {
        ...
        argv[1] = "./small_bunny_1080p_60fps.mp4";
    }
    ...
    // 1、AVCodecContext
    // 跟AVFormatContext类似，我们也是通过AVCodecContext对编码器Encoder和解码器Decoder操作，一般也不直接操作编解码器。
    // 所以需要实现编解码，一般都要跟AVCodecContext打交道。
...
// 由于解码器的输入是一个完整的帧数据包，而无论是网络传输还是文件读取，一般都是固定的buffer来读取的，
// 而不是安装格式的帧大小来读取，所以我们需要解析器Parser将流整理成一个一个的Frame数据包。
// 2、parser的全局声明在parsers.c，具体的定义在parser_list.c
